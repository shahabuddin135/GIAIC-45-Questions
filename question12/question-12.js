/*Greetings: Start with the array you used in Exercise 11,
 but instead of just printing each person’s name, print
 a message to them. The text of each message should be
 the same, but each message should be personalized with
 the person’s name.
*/
var greeting = ['Assalamualikum how are you', 'How are you doing', 'Nice to meet you', 'Best of Luck!'];
var MyFriends = ['Haris', 'Zahir', 'Arif', 'Shahab'];
for (var friend in MyFriends) {
    console.log("".concat(greeting[friend], " ").concat(MyFriends[friend]));
}
